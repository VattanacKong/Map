...
def bfs(origin, destination, districts, open =[], close= [], store = []): 
    for neighbor in districts[origin]:
        if (neighbor not in open) and (type(neighbor) is str) and(neighbor not in close):
    ...
    if open[0] == destination:
        ...
        next_start = open[0]
        open.remove(next_start)
        return bfs(next_start,destination,districts, open, close, store)
            
bfs('Tuol Kouk', "Kandal Stueng", districts)
